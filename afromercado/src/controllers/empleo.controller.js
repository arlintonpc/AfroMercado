...
const EmpleoController = {
  ...
  async listarPublicas(req, res, next) {
    try {
      const { municipio, departamento, categoria, tipoContrato, tipoPublicacion, search, salarioMin, salarioMax, page } = req.query;
      const data = await EmpleoService.listarPublicas({ municipio, departamento, categoria, tipoContrato, tipoPublicacion, search, salarioMin, salarioMax, page });
