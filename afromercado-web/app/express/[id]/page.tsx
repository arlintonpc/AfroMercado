...
function MenuExpressContent() {
  ...
  async function confirmarPedido() {
    ...
    return (
      <div className="min-h-screen bg-[#FAF8F5]">
        <div className="h-64 sm:h-80 lg:h-[400px] w-full bg-gray-200 animate-pulse" />
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex gap-4 mb-10 overflow-x-hidden">
            ...
            <div className="h-12 w-28 bg-gray-200 rounded-full animate-pulse hidden sm:block" />
            <div className="h-12 w-28 bg-gray-200 rounded-full animate-pulse hidden sm:block" />
            <div className="h-12 w-28 bg-gray-200 rounded-full animate-pulse hidden lg:block" />
          </div>
          <div className="lg:grid lg:grid-cols-[1fr_380px] gap-8">
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                {[1, 2, 3, 4, 5, 6].map(i => (
                  <div key={i} className="bg-white rounded-[1.75rem] border border-[#1A1A1A]/5 p-4 flex gap-4 h-[160px] sm:h-[340px] sm:flex-col animate-pulse">
                    <div className="w-28 h-28 sm:w-full sm:h-48 rounded-[1rem] sm:rounded-[1.25rem] bg-gray-200 flex-shrink-0" />
                    <div className="flex-1 flex flex-col justify-between pt-1">
                      <div>
                        <div className="h-4 bg-gray-200 rounded w-3/4 mb-2" />
                        <div className="h-3 bg-gray-200 rounded w-1/2 sm:w-full mb-1" />
                        <div className="h-3 bg-gray-200 rounded w-1/3 sm:w-4/5 hidden sm:block" />
                      </div>
                      <div className="h-6 w-24 bg-gray-200 rounded mt-4 ml-auto sm:ml-0" />
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="hidden lg:block">
              <div className="bg-white rounded-[2.5rem] border border-[#1A1A1A]/5 h-[400px] p-8 animate-pulse">
                ...
                <div className="h-14 w-full bg-gray-200 rounded-2xl mt-10" />
              </div>
            </div>
          </div>
        </div>
  ...
  if (paso === 'confirmado') {
    ...
    // Agrupar productos por sección y aplicar búsqueda
  const term = busqueda.toLowerCase().trim()
  const applySearch = (prods: MenuComercioExpress['productos']) => {
    if (!term) return prods
    return prods.filter(p => p.nombre.toLowerCase().includes(term) || (p.descripcion && p.descripcion.toLowerCase().includes(term)))
  }

  const secciones = menu?.secciones ?? []
  const sinSeccion = applySearch(menu?.productos?.filter(p => !p.menuSeccionId) ?? [])
  const conSeccion = secciones.map(s => ({
    seccion: s,
    productos: applySearch(menu?.productos?.filter(p => p.menuSeccionId === s.id) ?? []),
  })).filter(g => g.productos.length > 0)
  const hayTabs = secciones.length > 0

  const productosFiltradosBase = tabActiva === 'todos'
    ? (menu?.productos ?? [])
    : (menu?.productos?.filter(p => p.menuSeccionId === tabActiva) ?? [])
  const productosFiltrados = applySearch(productosFiltradosBase)
    ...
    <div className="min-h-screen bg-[#FAF8F5]">
      ...
      <header className="bg-white border-b border-gray-100">
        {/* Banner — video de la tienda si existe, si no slider de fotos, si no el logo */}
          ...
          <div className="absolute inset-0 -bottom-[30%] pointer-events-none will-change-transform" style={{ transform: `translateY(${scrollY * 0.4}px)` }}>
            {menu.videoUrl ? (
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          {hayTabs && (
                        <div className="sticky top-0 z-40 bg-white/80 backdrop-blur-xl border-b border-[#1A1A1A]/5 pb-3 pt-3 shadow-sm -mx-4 px-4 sm:mx-0 sm:px-0">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 w-full">
                <div className="overflow-x-auto scrollbar-hide flex-1">
                  <div className="flex gap-2 py-2 min-w-max">
                    <button
                      onClick={() => setTabActiva('todos')}
                      className={`flex-shrink-0 px-6 py-2.5 rounded-full text-sm font-bold transition-all shadow-sm border ${
                        tabActiva === 'todos'
                ...
                <div className="relative w-full sm:w-64 flex-shrink-0">
                  ...
                  <input
                    ...
                    className="block w-full pl-9 pr-3 py-2.5 sm:py-2 border border-gray-200 rounded-full text-sm placeholder-gray-400 focus:outline-none focus:border-[#1B4332] focus:ring-1 focus:ring-[#1B4332] bg-white/50 backdrop-blur-md shadow-inner transition-all"
                  />
                  {busqueda && (
                    <button onClick={() => setBusqueda('')} className="absolute inset-y-0 right-3 flex items-center text-gray-400 hover:text-gray-600">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 pb-40 lg:pb-8">
        ...
        <div className="space-y-6">
          ...
          <div className="text-center py-20 px-4">
            <div className="w-24 h-24 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-4 border border-[#1A1A1A]/5 shadow-sm">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-1">No encontramos &quot;{busqueda}&quot;</h3>
            <p className="text-gray-500 text-sm">Intenta buscar con otra palabra clave o revisa las categorías.</p>
            <button onClick={() => setBusqueda('')} className="mt-6 px-6 py-2 bg-[#F8F5F0] text-[#1B4332] font-semibold rounded-full hover:bg-gray-200 transition-colors">Limpiar búsqueda</button>
          </div>
        ) : tabActiva !== 'todos' ? (
