...
export default function ComercianteVitrinaPage() {
  ...
  const cargarComercio = useCallback(async () => {
    ...
    try {
      ...
      setComercio(c)
      if (!c) {
        setErrorComercio('Todavía no tienes una tienda registrada.')
      } else {
        const misProds = await listarMisProductos()
    ...
    <div className="mx-auto w-full max-w-2xl">
      <header className="mb-5 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="mt-1 text-[#1A1A1A]/65 text-sm">
            Comparte un video corto o fotos de tu negocio.
          </p>
        </div>
      </header>
