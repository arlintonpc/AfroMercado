...
function AjustarVista({ hoteles, userLat, userLon }: { hoteles: ConfigHotel[]; userLat: number | null; userLon: number | null }) {
  const map = useMap()
  useEffect(() => {
    const puntos: [number, number][] = hoteles
      .filter(h => h.comercio.latitud && h.comercio.longitud)
      .map(h => [h.comercio.latitud!, h.comercio.longitud!])
    ...
    <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-sm h-[50vh] min-h-[280px] max-h-[400px]">
      <MapContainer center={userLat && userLon ? [userLat, userLon] : centro} zoom={6} style={{ height: '100%', width: '100%' }} scrollWheelZoom={false}>
        ...
        {conCoordenadas.map(hotel => {
          ...
          return (
            <Marker key={hotel.id} position={[lat, lon]} icon={iconoHotel}
              ref={m => { if (m) markerRefs.current[hotel.id] = m }}
            >
              <Popup minWidth={220}>
        ...
        {conCoordenadas.length === 0 && (
          <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', background: 'white', padding: '12px 20px', borderRadius: 12, boxShadow: '0 2px 12px rgba(0,0,0,.15)', zIndex: 1000, textAlign: 'center', fontSize: 13, color: '#666' }}>
            ...
            Los hoteles aún no tienen ubicación registrada
          </div>
        )}
      </MapContainer>
    </div>
