...
export default function ModalDenunciarInmueble({
  ...
  const [motivo, setMotivo] = useState<MotivoDenunciaInmueble | ''>('')
  ...
  async function handleEnviar() {
    ...
    setError(null)
    try {
      await denunciarInmueble(inmuebleId, {
        motivo,
      ...
      <div className="bg-white rounded-2xl border border-[#1A1A1A]/8 w-full max-w-md shadow-xl max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-semibold text-[#1A1A1A]">Reportar esta publicación</h3>
            <button
              onClick={onCerrar}
          ...
          {exito ? (
            <div className="text-center py-6">
              ...
              <p className="text-sm text-[#1A1A1A]/55 mb-5">
                Un administrador revisará esta publicación. Gracias por ayudarnos a cuidar la comunidad.
              </p>
              <button
                type="button"
                onClick={onCerrar}
            ...
            <>
              ...
              <div className="flex flex-col gap-2 mb-3">
                {MOTIVOS_DENUNCIA_INMUEBLE.map((m) => (
                  <label
                    key={m.value}
                  ...
                  >
                    <input
                      type="radio"
                      name="motivo-denuncia-inmueble"
                      value={m.value}
                      checked={motivo === m.value}
                      onChange={() => setMotivo(m.value)}
                      className="accent-[#2D6A4F]"
                    />
                    {m.value === 'ESTAFA_DINERO' ? 'Piden dinero por adelantado (estafa)' : m.label}
                  </label>
                ))}
              </div>
