...
interface Props {
  reviews: ReviewItem[]
  cargando?: boolean
  /** id de la reserva/pedido elegible para reseñar (undefined = no puede reseñar) */
  elegibleId?: number
  /** Texto del placeholder del textarea */
  placeholder?: string
  /** Callback al enviar. Recibe calificacion y comentario, devuelve la nueva reseña */
  onCrear?: (elegibleId: number, calificacion: number, comentario?: string) => Promise<ReviewItem>
  onNueva?: (r: ReviewItem) => void
}
...
export default function SeccionReviews({
  ...
  const yaDejo = usuario && reviews.some(r => String(r.clienteId) === String(usuario.id))

  async function enviar() {
    if (!elegibleId || !onCrear) return
    setEnviando(true)
    setError('')
    try {
      const nueva = await onCrear(elegibleId, calificacion, comentario.trim() || undefined)
      onNueva?.(nueva)
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-4 mt-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="font-semibold text-[#1A1A1A]">Reseñas</h2>
          {reviews.length > 0 && (
        ...
        {elegibleId && !yaDejo && usuario && onCrear && (
          <button
          ...
          {reviews.map(r => (
            <div key={r.id} className="border-b border-gray-50 pb-4 last:border-0 last:pb-0">
              <div className="flex items-center gap-2 mb-1">
                ...
                <div>
                  <p className="text-xs font-semibold text-[#1A1A1A]">{r.cliente?.nombre ?? 'Viajero'}</p>
