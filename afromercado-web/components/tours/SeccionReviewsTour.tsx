...
import { reviewsTour, crearReviewTour, type ReviewTour } from '@/lib/api/review'
...
export default function SeccionReviewsTour({ configTourId, reservaElegibleId }: Props) {
  const { usuario } = useAuth()
  const [reviews, setReviews] = useState<ReviewTour[]>([])
  ...
  useEffect(() => {
    reviewsTour(configTourId).then(r => { setReviews(r); setCargando(false) })
  }, [configTourId])
  ...
  async function enviarReview() {
    ...
    try {
      const nueva = await crearReviewTour(reservaElegibleId, calificacion, comentario || undefined)
