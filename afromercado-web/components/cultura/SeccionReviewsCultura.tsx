...
import { reviewsCultura, crearReviewCultura, type ReviewCultura } from '@/lib/api/review'
...
export default function SeccionReviewsCultura({ eventoCulturalId, reservaElegibleId }: Props) {
  ...
  useEffect(() => {
    reviewsCultura(eventoCulturalId).then(r => { setReviews(r); setCargando(false) })
  }, [eventoCulturalId])
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-4 mt-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h2 className="font-semibold text-[#1A1A1A]">Reseñas del evento</h2>
          {reviews.length > 0 && (
        ...
        <div className="bg-gray-50 rounded-xl p-4 mb-4 space-y-3">
          ...
          <div>
            ...
            <textarea value={comentario} onChange={e => setComentario(e.target.value)} rows={3}
              placeholder="¿Cómo fue tu experiencia en el evento?"
