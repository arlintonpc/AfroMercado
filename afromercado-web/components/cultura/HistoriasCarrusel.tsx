            ...
            : grupos.map((grupo, indice) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={`absolute left-2 top-2 h-8 w-8 rounded-full p-[2px] transition-transform duration-300 group-hover:scale-105 ${grupo.vistasTodas ? 'bg-white/50' : 'bg-gradient-to-tr from-[#2D6A4F] via-[#D4A017] to-[#F4C842] shadow-md'}`}>
                      <div className="h-full w-full overflow-hidden rounded-full border border-white bg-[#1B4332]">
                        {avatar ? (
                          /* eslint-disable-next-line @next/next/no-img-element */
                          <img src={avatar} alt="" className="h-full w-full object-cover" />
                        ) : <span className="grid h-full w-full place-items-center text-[10px] font-black text-white">{grupo.nombre.charAt(0)}</span>}
                      </div>
                    </div>
                    <span className="absolute inset-x-2 bottom-2 line-clamp-2 text-[10px] font-bold leading-tight text-white drop-shadow-md">{grupo.nombre}</span>
                  </button>
                )
              })}
        </div>
